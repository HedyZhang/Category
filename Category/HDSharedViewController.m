...
@interface ButtonActionItem ()
@property (nonatomic, copy) NSString *text;

@property (nonatomic, strong) UIImage *image;

@property (nonatomic, strong) ButtonActionItemHandler handler;
@end

@implementation ButtonActionItem
...
- (void)setup
{
    ...
    self.translatesAutoresizingMaskIntoConstraints = NO;//标示是否自动布局约束 default YES；
...
- (void)addButtonView:(ButtonActionItem *)buttonView;
...
- (void)prepareForShow
{
    ...
    for (int i = 0; i < [self.buttonArray count]; i++) {
        ButtonActionItem *buttonView = [self.buttonArray objectAtIndex:i];
...
@interface HDSharedViewController ()
{
    UIView *_snapshotView;
}
@end

@implementation HDSharedViewController
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    [self cutSnapshotOfPresentingView];
}
...
/**
 *  获取模态之前页面的屏幕快照，仅适用iOS7
 */
- (void)cutSnapshotOfPresentingView
{
    UIView *presentingView = self.presentingViewController.view;
    _snapshotView = [presentingView snapshotViewAfterScreenUpdates:YES];
    [[UIApplication sharedApplication].keyWindow addSubview:_snapshotView];
}
